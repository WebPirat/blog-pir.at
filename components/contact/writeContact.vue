...
const submitForm = async () => {
  try {
    ...
    console.log('Comment:', comment.value);
      supabase.from('blog_contact').insert([
      {username: name.value, content: comment.value, email: email.value}
    ]).select('id').then((data) => {
      console.log(data);
      if(data.error){
...
<template>
  <div>
    <div>
      <h2 v-if="!send" class="text-2xl font-bold mb-4 mt-6">Nachricht verschicken:</h2>
      <div class="p-2 m-2">
        <valid-message v-if="send" :show="send" :message="errorMessage" :type="errortype" component="writeGuestbookComment" />
      </div>
      <div>
        <input v-model="name" type="text" placeholder="Name" class="border border-lightgray p-2 m-2 w-full" required />
        <input v-model="email" type="email" placeholder="E-Mail" class="border border-lightgray p-2 m-2 w-full" />
        <textarea v-model="comment" placeholder="Comment" class="border border-lightgray p-2 m-2 w-full" required></textarea>
