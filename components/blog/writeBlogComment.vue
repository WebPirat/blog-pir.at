...
const submitForm = async () => {
  try {
    ...
    console.log('Comment:', comment.value);
      supabase.from('blog_posts_comments').insert([
      {username: name.value, content: comment.value, pID: props.blogid}
    ]).select('id').then((data) => {
      console.log(data);
        send.value = true;
...
<template>
  <div>
    <div>
      <h2 class="text-2xl font-bold mb-4 py-2 mt-6">Kommentar schreiben:</h2>
      <valid-message v-if="send" :show="send" :message="errorMessage" :type="errortype" component="writeBlogComment" />
      <div>
